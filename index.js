 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function Rr({appConfig:e,heartbeatServiceProvider:t},n){const r=Pr(e,n),o=br(e,n),i=t.getImmediate({optional:!0});if(i){const u=await i.getHeartbeatsHeader();u&&o.append("x-firebase-client",u)}const s={installation:{sdkVersion:ot,appId:e.appId}},a={method:"POST",headers:o,body:JSON.stringify(s)},c=await lt(()=>fetch(r,a));if(c.ok){const u=await c.json();return ct(u)}else throw await ut("Generate Auth Token",c)}function Pr(e,{fid:t}){return`${at(e)}/${t}/authTokens:generate`}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function Po(e=Kn()){return Mo().then(t=>{if(!t)throw f.create("unsupported-browser")},t=>{throw f.create("indexed-db-unsupported")}),fe(x(e),"messaging").getImmediate()}async function kt(e,t){return e=x(e),Ct(e,t)}function Lo(e){return e=x(e),$o(e)}function Fo(e,t){return e=x(e),Ro(e,t)}Bo();const xo={apiKey:"AIzaSyAH2JIEEwn9GfYpfzlTpvUW4s0LD_iRwJQ",authDomain:"silbo-patient-transport.firebaseapp.com",projectId:"silbo-patient-transport",storageBucket:"silbo-patient-transport.firebasestorage.app",messagingSenderId:"27523190820",appId:"1:27523190820:web:6dcd16ac7bf6ff72e008b1"},jo="BFs1O-6MsXDwy0GEZwqDZiKaToqjiAJ23MIuZTYBtj8k0Psr9NqTv4CTwlQJY5oCJYR4RmO7FhsMfeS0k0iMaBs";function Ho(){return Xe(xo)}Ho();const $=Po(),Ot="token_div",Nt="permission_div";Fo($,e=>{console.log("Message received. ",e),qo(e)});async function Se(){zo(),R("loading..."),kt($,{vapidKey:jo,serviceWorkerRegistration:await navigator.serviceWorker.ready}).then(e=>{e?(Vo(e),Go(e)):(console.log("No registration token available. Request permission to generate one."),Jo(),P(!1))}).catch(e=>{console.log("An error occurred while retrieving token. ",e),R("Error retrieving registration token."),P(!1)})}function R(e){const t=document.querySelector("#token");t.textContent=e}function Vo(e){Ko()?console.log("Token already sent to server so won't send it again unless it changes"):(console.log("Sending token to server...",e),P(!0))}function Ko(){return window.localStorage.getItem("sentToServer")==="1"}function P(e){window.localStorage.setItem("sentToServer",e?"1":"0")}function L(e,t){const n=document.querySelector("#"+e);t?n.style.display="block":n.style.display="none"}function Uo(){console.log("Requesting permission..."),Notification.requestPermission().then(e=>{console.log("Permission: ",e),e==="granted"?(console.log("Notification permission granted."),Se()):console.log("Unable to get permission to notify.")})}function Wo(){kt($).then(e=>{Lo($).then(()=>{console.log("Token deleted.",e),P(!1),Se()}).catch(t=>{console.log("Unable to delete token. ",t)})}).catch(e=>{console.log("Error retrieving registration token. ",e),R("Error retrieving registration token.")})}function qo(e){const t=document.querySelector("#messages"),n=document.createElement("h5"),r=document.createElement("pre");r.style.overflowX="hidden;",n.textContent="Received message:",r.textContent=JSON.stringify(e,null,2),t.appendChild(n),t.appendChild(r)}function zo(){const e=document.querySelector("#messages");for(;e.hasChildNodes();)e.removeChild(e.lastChild)}function Go(e){L(Ot,!0),L(Nt,!1),R(e)}function Jo(){L(Ot,!1),L(Nt,!0)}document.getElementById("request-permission-button").addEventListener("click",Uo);document.getElementById("delete-token-button").addEventListener("click",Wo);Se();
